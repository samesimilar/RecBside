...
t_int hid_open_device(t_hid *x, t_symbol *device_name)
{
	...
	x->x_device_name = device_name;
	if (!device_name) {
		return 1;
	}
	sprintf(dev_handle_name,"/dev/input/by-id/%s",x->x_device_name->s_name);
	
	// need to make sure device path is linked to an 'eventn' file, otherwise it can crash
	char buf[256];
...
// ../event
  ...
  if (dev_handle_name) 
  {
	  ...
	  ioctl(x->x_fd, EVIOCGNAME(sizeof(device_output_string)), device_output_string);
	  t_symbol *device_output_symbol = gensym(device_output_string);
	  outlet_symbol(x->x_device_name_outlet, device_output_symbol);
  } 
